...
public class PlayerInsertion {
    ...
    private static void insertPlayers(String json, int teamId, int seasonId) {
        try (Connection connection = DriverManager.getConnection(JDBC_URL, USERNAME, PASSWORD)) {
            ...
            String playerInsertQuery = "INSERT INTO player (id_player, firstname, lastname, date_of_birthday, country, started_year, pro, hight, weight, college, affiliation) " +
                    "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE id_player=id_player";
    ...
    private static void insertPlayer(Connection connection, PreparedStatement playerStatement, JSONObject playerObject, int teamId, int seasonId) throws SQLException {
        ...
        // Aggiunta di country
        setNullableString(playerStatement, 5, birthObject, "country");
    ...
    private static void insertPlayerDetails(Connection connection, int playerId, int teamId, JSONObject playerObject) throws SQLException {
        String playerDetailsInsertQuery = "INSERT INTO player_details (id_player, id_team, season, pos, active, shirt_number) " +
                "VALUES (?, ?, ?, ?, ?, ?)";
